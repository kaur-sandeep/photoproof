...
$(document).ready(function() {
    let table = $('#userTableList').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: window.APP_URL + '/admin/users/list/two',
            data: function (d) {
                d.organization_id = getQueryParam('organization_id'); // sent only if present in URL
            }
        },
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
    ...
    // Small helper to read ?organization_id=5 from the current page URL
...
//         serverSide: true,
//         ajax: {
//             url: window.APP_URL + '/admin/photos/list',
//             data: function (d) {
//                 d.organization_id = new URLSearchParams(window.location.search).get('organization_id');
//             }
//         },
...
//                 searchable: false,
//                 render: function(data, type, row) {
//                     return `<button class="btn btn-info btn-sm viewTrackBtn">View</button>`;
//                 }
//             },
//         ]
//     });

//     // ... rest of your existing code (viewTrackBtn, toggle-state, delete-user) stays exactly the same
// });
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (!response.success) {
                        ...
                        return; // reload mat karo, status change nahi hua
                    }
...
$(document).ready(function() {
    let table = $('#employeesList').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URL + '/owner/employee/list/',  // Ensure this URL is correct
    ...
    // STATUS TOGGLE
$('#employeesList').on('click', '.toggle-status', function () {
    let id = $(this).data('id');
    let status = $(this).data('status');
    if (confirm("Are you sure you want to change the status?")) {
       $.ajax({
    url: window.APP_URL + '/owner/update/employee/status',
    type: "get",
    data: {
        id: id,
        status: status
    ...
    // DELETE USER
    $('#employeesList').on('click', '.delete-user', function () {
        let id = $(this).data('id');
        if (confirm("Are you sure you want to delete this employee?")) {
            $.get(window.APP_URL + '/owner/update/employee/status', {
                id: id,
                status: -1
            }, function () {
                table.ajax.reload(null, false);
...
$(document).ready(function() {
    let table = $('#empphotodataTableList').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URL + '/owner/fetch/employee/images/' +  window.USER_ID,
        columns: [
            { data: 'serial_number', name: 'serial_number' },
              {
                data: 'images',
        ...
        $(document).on('click', '.viewTrackBtn', function () {
            ...
            var table = $('#empphotodataTableList').DataTable();
    ...
    $('#empphotodataTableList').on('click', '.toggle-state', function () {
        let id = $(this).data('id');
        let state = $(this).data('state');
        if (confirm("Are you sure you want to change the status?")) {
        $.ajax({
...
$(document).ready(function() {
    ...
    // STATUS TOGGLE
$('#organizationemployeesList').on('click', '.toggle-status', function () {
    let id = $(this).data('id');
    let status = $(this).data('status');
    if (confirm("Are you sure you want to change the status?")) {
       $.ajax({
    ...
    // DELETE USER
    $('#organizationemployeesList').on('click', '.delete-user', function () {
        let id = $(this).data('id');
        if (confirm("Are you sure you want to delete this employee?")) {
            $.get(window.APP_URL + '/owner/update/employee/status', {
                id: id,
